...
public class gravityPath : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float gravityComponent = Vector3.Dot(gravity.normalized * gateSpeedMultiplier, direction.normalized);
        velocity += gravityComponent * gravity.magnitude * gateSpeedMultiplier * Time.deltaTime;
        ...
        {
            if (audioSource && Mathf.Abs(velocity) > 1 && canPlaySound == true)
            {
